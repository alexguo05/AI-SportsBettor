...
class PriceRepository:
    ...
    @classmethod
    def from_environment(cls, src_dir: Path) -> PriceRepository:
        return cls(create_database_resources(src_dir))
        ...
        with self.resources.engine.connect() as connection:
            row = (
                connection.execute(
                    ingest_cursors.select().where(
                        ingest_cursors.c.source == POLYMARKET_CURSOR_SOURCE,
                        ingest_cursors.c.stream == "clob_price_history",
                    )
                )
                .mappings()
                .one_or_none()
    ...
    def load_price_cursors(self, token_ids: list[str]) -> dict[str, dict[str, Any]]:
        ...
        with self.resources.engine.connect() as connection:
            rows = (
                connection.execute(
                    select(polymarket_price_cursors).where(
                        polymarket_price_cursors.c.token_id.in_(token_ids)
                    )
                )
                .mappings()
                .all()
    ...
    def persist_records(self, envelope: dict[str, Any]) -> None:
        with self.resources.engine.begin() as connection:
            ...
            points = envelope["records"]
            for offset in range(0, len(points), 1_000):
                normalized_points: list[dict[str, Any]] = []
                for point in points[offset : offset + 1_000]:
                    ...
                    normalized_points.append(
                        {
                            ...
                            "first_observed_at": observed_at,
                            "last_observed_at": observed_at,
                            "latest_raw_ingest_run_id": envelope["ingest_run_id"],
                        }
                    )
                keys = [
                    (point["token_id"], point["source_timestamp"])
                    for point in normalized_points
                ]
                existing_rows = (
                    connection.execute(
                        select(
                            polymarket_price_points.c.token_id,
                            polymarket_price_points.c.source_timestamp,
                            polymarket_price_points.c.price,
                            polymarket_price_points.c.fidelity_minutes,
                        ).where(
                            tuple_(
                                polymarket_price_points.c.token_id,
                                polymarket_price_points.c.source_timestamp,
                ...
                existing = {
                    (row["token_id"], row["source_timestamp"]): row
                ...
                for point in normalized_points:
                    ...
                    if previous is not None and (
                        ...
                        and previous["fidelity_minutes"]
                        == point["fidelity_minutes"]
                    ):
                        continue
                    version_rows.append(
                        {
                ...
                if version_rows:
                    connection.execute(
                        insert(polymarket_price_point_versions)
                        .values(version_rows)
                        .on_conflict_do_nothing(
                            index_elements=[
                                polymarket_price_point_versions.c.token_id,
                                polymarket_price_point_versions.c.source_timestamp,
                                polymarket_price_point_versions.c.observed_at,
                            ]
                        )
                    )
                point_insert = insert(polymarket_price_points).values(
                    normalized_points
                )
                connection.execute(
                    point_insert.on_conflict_do_update(
                        index_elements=[
                            polymarket_price_points.c.token_id,
                            polymarket_price_points.c.source_timestamp,
                        ],
                        set_={
                            "price": point_insert.excluded.price,
                            "fidelity_minutes": point_insert.excluded.fidelity_minutes,
                            "first_observed_at": func.least(
                                polymarket_price_points.c.first_observed_at,
                                point_insert.excluded.first_observed_at,
                            ),
                            "last_observed_at": point_insert.excluded.last_observed_at,
                            "latest_raw_ingest_run_id": (
                                point_insert.excluded.latest_raw_ingest_run_id
                            ),
                        },
                        where=(
                            point_insert.excluded.last_observed_at
                            >= polymarket_price_points.c.last_observed_at
                        ),
                    )
                )

            cursor_updated_at = _timestamp(
            ...
            token_cursor_rows = [
                {
                    "token_id": token_id,
                    "query_fingerprint": envelope["request"]["query_fingerprint"],
                    "last_end_ts": end_ts,
                    "updated_at": cursor_updated_at,
                }
                for token_id, end_ts in envelope["_token_cursor_candidates"].items()
            ]
            for offset in range(0, len(token_cursor_rows), 1_000):
                token_cursor_insert = insert(polymarket_price_cursors).values(
                    token_cursor_rows[offset : offset + 1_000]
                )
                connection.execute(
                    token_cursor_insert.on_conflict_do_update(
                        index_elements=[polymarket_price_cursors.c.token_id],
                        set_={
                            "query_fingerprint": (
                                token_cursor_insert.excluded.query_fingerprint
                            ),
                            "last_end_ts": token_cursor_insert.excluded.last_end_ts,
                            "updated_at": token_cursor_insert.excluded.updated_at,
                        },
                        where=(
                            token_cursor_insert.excluded.last_end_ts
                            >= polymarket_price_cursors.c.last_end_ts
                        ),
                    )
                )
